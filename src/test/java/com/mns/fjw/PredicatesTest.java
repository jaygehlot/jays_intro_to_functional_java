...
public class PredicatesTest {
    ...
    public void javaStreamPredicate_exercise1() {
        List<String> names = asList("bob", "fred", "john", "jaygehlot");
         ...
         */
        Predicate<String> lengthIsFourOrMore = string -> string.length() >= 4;
        assertThat(
                names.stream().filter(lengthIsFourOrMore).collect(toList()),
                hasExactly("fred", "john", "jaygehlot")
        );
    }
    ...
    */
    ...
    public void totallyLazyPredicate_exercise2() {
        ...
        assertThat(
                sequenceOfNames.filter(n -> n.length() >= 4), //replace with your four or more filter
    ...
    public void backToPredicates_exercise3() {
        ...
        com.googlecode.totallylazy.predicates.Predicate<Person> males = p -> p.sex.equals("M");
        assertThat(
                people.filter(males),
                hasExactly(bob, john)
        );
        ...
        com.googlecode.totallylazy.predicates.Predicate<Person> livingInLondon = p -> p.livesInCity.equals("London");
        assertThat(
...
//        // any idea how to find males living in london?
        //use AND and OR in the Predicate interface
        //or you can define two Predicates and apply two filters
        assertThat(
                people.filter(males.and(livingInLondon)),
                hasExactly(bob)
        );

//        // how about females living outside of london?
        assertThat(
                people.filter(not(livingInLondon).and(not(males))),
                hasExactly(jackie)
        );
    }
